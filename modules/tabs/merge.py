...
def tab():
    ...
    def update_speaker_ids(model):
        ...
        model = torch.load(
            os.path.join(models.MODELS_DIR, "checkpoints", model), map_location="cpu"
        )
    ...
    with gr.Group():
        with gr.Column():
            ...
            with gr.Row().style(equal_height=False):
                merge_and_save_button = gr.Button("Merge and save", variant="primary")
            ...
            model_a.change(update_speaker_ids, inputs=[model_a], outputs=[speaker_id])
